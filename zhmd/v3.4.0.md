...
为在采样速度与放大（USDU：Ultimate SD Upscale）时的显存饱和 / 系统内存溢出之间取得平衡，采样器（`HSWQSampler`）与放大节点（`HSWQUltimateSDUpscale`）上分别提供独立的 **BOOLEAN 开关**。
...
| **额外显存** | **0 MB**（无额外占用） | 约 1.2 GB–1.5 GB（单一固定形状） |
| **CPU launch 延迟** | 存在（每层 15–30 μs） | **为零**（GPU 批量重放） |
| **速度** | 基线 | **最快（约快 15%–25%）** |
| **推荐用途** | **· USDU 分块放大<br>· ≤16 GB 显存<br>· 输入形状变化（tiling）** | **· ≥24 GB 显存<br>· 首遍 1024×1024 单一分辨率<br>· 以最高速度连续采样** |

### 3.2 USDU（分块放大）显存膨胀与缓解
在多图块 USDU 中，边缘处理等情况会按图块喂入 **不同的输入形状（$M$）**。
PyTorch CUDA Graphs 按形状各捕获一张图，因此在 16 GB 显卡上捕获的 arena 会堆叠——专用显存接近 15.5 GB 饱和，且超过 15 GB 会溢出到共享 GPU 内存（系统 RAM）。

在 USDU 节点上将 `tensor_boost = False`（默认）会在放大一开始就清空 CUDA Graph 缓存，并以 **0 MB 额外显存的 Eager Pooled** 运行。
...
2. **`HSWQ Sampler`（首遍采样节点）**：
   ...
   - **ON（`True`）**：首遍 1024×1024 采样以完整 Tensor Boost（CUDA Graph）速度运行。

3. **`HSWQ Ultimate SD Upscale`（USDU 节点）**：
   - **`tensor_boost`（BOOLEAN 开关）**（`default: False`）。
   - **OFF（`False`）**：放大开始时设置 `HSWQ_NVFP4_TENSORBOOST=0` 并运行 `clear_nvfp4_cudagraphs()`，以 0 MB 额外显存防止内存溢出。
...
| 符号 | 含义 |
|:---|:---|
| `_ACT_Q_POOL` / `_ROT_OUT_POOL` | Eager 激活量化 / Hadamard 旋转缓冲区复用（约 0 额外显存） |
...
| CG OFF（环境变量未设 / `0`） | Eager pooled（约 0 额外显存） |
...
2. `HSWQ Sampler` 设 `tensor_boost = True` → 加速固定分辨率底图生成。
3. `HSWQ Ultimate SD Upscale` 设 `tensor_boost = False`（默认）→ 在图块前丢弃图；安全跑 Eager。
4. 在 ≤16 GB 或形状高度变化时，Sampler 也保持 False。
5. 在 ≥24 GB 且单一分辨率连续采样时，优先 Sampler True。
